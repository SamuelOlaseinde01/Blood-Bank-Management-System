...
import { addBloodStock } from "./admin-api";
...
export async function action({request}) {
    try {
        ...
        const msg = await addBloodStock(data)
        toast(msg)
    ...
    return (
        <>
            ...
            <Form method="post" className="add-blood-form">
                ...
                <button className={navigation.state === "submitting" ? "submitting-btn" : "submit-btn"} disabled={navigation.state ==="submitting"}>{ navigation.state === "submitting" ? "Adding blood...": "Add Blood"}</button>
            </Form>
            </div>
            </div>
        </>
