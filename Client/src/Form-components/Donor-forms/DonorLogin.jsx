...
import { Link, Form, useLoaderData, useNavigation, redirect } from "react-router-dom";
import { Home, Visibility, VisibilityOff } from "@mui/icons-material";
import { donorPostLogin } from "./donorApi";
...
export async function action({request}) {
  ...
  try{
    const username = await donorPostLogin(data)
    return redirect(`/donor?username=${username}&ts=${Date.now()}`)
  } catch (err) {
  ...
  return (
    <div className="login-container">
      ...
      <Form method="post" className="login-form">
        ...
        <Link style={{textDecoration : "none", color: "red"}} to={"/verify-donor-username"}>Forgot password?</Link>
        ...
        <div className="no-account">
            <p>Don't have an account? <Link style={{textDecoration: "none", color: "red"}} to={"/donor-register"}>Sign up</Link></p>
        </div>
      </Form>
      </div>
    </div>
